...
Driver native GPS uses `@capacitor/geolocation` (permissions + high-accuracy watch). After dependency changes run `npm --prefix "Dripless Driver" run mobile:sync`.
...
| Press Home (app backgrounded); wait 60s; Ops still receives ≥1 GPS tick | ☐ | May fail without foreground service |
...
**Pass criteria:** foreground GPS works on both phones. Background/lock-screen rows should pass; if they fail, record it — true screen-off tracking needs a foreground-service / background-geolocation plugin beyond Capacitor Geolocation.
