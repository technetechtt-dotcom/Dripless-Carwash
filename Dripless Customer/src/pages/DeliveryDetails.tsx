...
import { ArrowLeftIcon, MapPinIcon, PackageCheckIcon, ShieldCheckIcon } from 'lucide-react';
...
const DeliveryDetails = () => {
  ...
  const pickupPoint = useMemo(() => (bookingDetails?.pickupCoordinates as GeoPoint | null) || textToGeoPoint(pickup, 13), [bookingDetails, pickup]);
  const destinationPoint = useMemo(() => (bookingDetails?.destinationCoordinates as GeoPoint | null) || textToGeoPoint(destination, 17), [bookingDetails, destination]);
  ...
  return <main className="min-h-screen pb-24 bg-slate-50 dark:bg-slate-950">
    <header className="sticky top-0 z-20 glass-nav px-4 py-4 flex items-center">
      ...
      <h1 className="text-xl font-bold dark:text-white">Review delivery</h1>
    </header>
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 max-w-xl mx-auto space-y-5">
      ...
      <div className="glass-card p-5 dark:bg-slate-800/90 space-y-4">
        <div className="flex gap-3"><MapPinIcon className="text-eco-600 shrink-0" /><div><p className="text-xs text-slate-500">Collection</p><p className="font-bold dark:text-white">{pickup}</p><p className="text-xs text-slate-500 mt-2">Delivery</p><p className="font-bold dark:text-white">{destination}</p></div></div>
        <div className="flex gap-3 border-t border-slate-200 dark:border-slate-700 pt-4"><PackageCheckIcon className="text-eco-600" /><span className="text-sm text-slate-600 dark:text-slate-300">Package details are confirmed with the assigned courier.</span></div>
      ...
      <div className="glass-card p-4 flex gap-3 dark:bg-slate-800/90"><ShieldCheckIcon className="text-eco-600 shrink-0" /><p className="text-sm text-slate-600 dark:text-slate-300">A verified courier is assigned after the booking and payment are created. Courier details will appear in live tracking.</p></div>
